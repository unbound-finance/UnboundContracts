...
library OracleLibrary {
    ...
    function checkBaseAssetValue(address baseAssetAddr, uint8 percentDiff)
        internal
        view
    {
        (, int256 price, , , ) =
            AggregatorV3Interface(baseAssetAddr).latestRoundData();
    ...
    function checkBaseAssetValueTriangulate(
        ...
        (, int256 price, , , ) =
            AggregatorV3Interface(baseAssetAddr).latestRoundData();
        (, int256 price2, , , ) =
            AggregatorV3Interface(secondBaseAsset).latestRoundData();
        ...
        uint256 finalPrice =
            _baseAssetValue.mul(_secondBaseAsset).div(10**(toNormalize));
    ...
    function getLatestPrice(address assetAddr) public view returns (uint256) {
        (, int256 price, , , ) =
            AggregatorV3Interface(assetAddr).latestRoundData();
        return uint256(price);
    }

    // Returns latest price from ChainLink Oracle (Triangulation)
    function getLatestPriceTriangulate(
        address erc20PriceAddr,
        address ethPriceAddr
    ) public view returns (uint256) {
        // bat price in ETH
        (, int256 price, , , ) =
            AggregatorV3Interface(erc20PriceAddr).latestRoundData();

        // ETH price in USD
        (, int256 price2, , , ) =
            AggregatorV3Interface(ethPriceAddr).latestRoundData();
    ...
    function normalizeTo8(uint256 price, uint256 _theDecimal)
        private
        view
    ...
    function checkBaseAssetPrices(
        ...
        if (_triangulateBaseAsset) {
            checkBaseAssetValueTriangulate(
                _addresses[0],
                _addresses[1],
                _maxPercentDiffBaseAsset
            );
        } else {
