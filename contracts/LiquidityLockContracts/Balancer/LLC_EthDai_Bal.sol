...
contract LLC_EthDai_Bal {
    ...
    function getValue() internal view returns (uint256 _totalAsset) {
        // obtain normalized weight of baseAsset.
        uint256 baseAssetWeight = LPTContract.getNormalizedWeight(_baseAsset);

        // Obtain total amount of baseAsset
        uint256 baseAssetTotal = LPTContract.getBalance(_baseAsset);
        ...
        if (baseAssetDecimal != 18) {
            ...
            if (baseAssetDecimal < 18 && baseAssetDecimal >= 0) {
                ...
                baseAssetTotal = baseAssetTotal * (10 ** uint256(difference));
            }
        ...
        _totalAsset = baseAssetTotal.mul(10 ** 18).div(baseAssetWeight); 
